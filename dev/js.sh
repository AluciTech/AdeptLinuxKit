...
Script to easily install nvm node version manager
...
OPTS=$(getopt -o hd --long help,debug -n'js.sh' -- "$@")
...
#Installation
eval "curl $MUTE_CURL -o- https://raw.githubusercontent.com/nvm-sh/nvm/v0.40.2/install.sh | bash $REDIRECT"
export NVM_DIR=$REAL_HOME/.nvm
source $NVM_DIR/nvm.sh
eval "nvm --version $REDIRECT"
...
if [ $exitcode -eq 0 ]; then
	echo -e "${GREEN}js sucessfully installed${RESET}"
else
	echo -e "${RED}Error while installing js${RESET}"
fi
